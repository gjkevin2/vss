...
cargo --version 2>/dev/null||{
    ...
    echo 'export RUSTUP_UPDATE_ROOT=https://mirrors.ustc.edu.cn/rust-static/rustup' >> $HOME/.cargo/env
    echo 'export RUSTUP_DIST_SERVER=https://mirrors.ustc.edu.cn/rust-static' >> $HOME/.cargo/env
...
#install nodejs for rust
sudo pacman -Syu --noconfirm --needed npm clang bear
...
npm install coc-snippets coc-rust-analyzer coc-json coc-pairs coc-clangd --global-style --ignore-scripts --no-bin-links --no-package-lock --only=prod

# 子shell执行异步命令
# (vim +"CocInstall coc-snippets coc-rust-analyzer")

# 修改rust-analyzer下载地址
modjs=~/.config/coc/extensions/node_modules/coc-rust-analyzer/lib/index.js
# wait until cocinstall finished
while [ ! -f $modjs ]
do
    ping -c 2 127.0.0.1 >/dev/null
done
sed -i "s#(release.url#('https://ghproxy.com/'+release.url#" ~/.config/coc/extensions/node_modules/coc-rust-analyzer/lib/index.js
# 配置analyzer
...
func CheckCode()
    exec "w"
    exec 'AsyncRun -cwd=<root> cargo check'
endfunc
" normal mode check
nnoremap <silent> <F6> :call CheckCode() <cr>
" insert mode check
inoremap <silent> <F6>  <ESC> :call CheckCode() <cr>
...
endfunc
inoremap <silent> <F7> <ESC> :call Buildmore() <cr>
nnoremap <silent> <F7> :call Buildmore() <cr>
" Ctrl-F7 编译发布版本
func Releasemore()
    exec "w"
    if &filetype == "cpp"
    ...
    elseif &filetype == "c"
        ...
        exec 'AsyncRun -cwd=<root> grep "release]" Cargo.toml >/dev/null ||echo -e "\n[profile.release]\nstrip=true\nlto=true\nopt-level=\"z\"\ncodegen-units=1\npanic=\"abort\"">>Cargo.toml && cargo build --release'
    endif
endfunc
nnoremap <silent> <C-F7> :call Releasemore() <cr>
"F8 运行当前项目
func Runmore()
    exec "w"
    if &filetype == "cpp"
