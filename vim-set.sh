...
mkdir -p ~/.vim/pack/colors/{opt,start}
mkdir -p ~/.vim/pack/syntax/{opt,start}
cd ~/.vim/pack
...
install_plugin(){
    ...
    if [ ! -d "~/.vim/pack/$2/start/$plugin_name" ]; then
        git submodule add https://github.com/$1 $2/start/$plugin_name
    else
        git submodule update
    fi
    vim -u NONE -c "helptags ~/.vim/pack/$2/start/$plugin_name/doc" -c q
}
install_plugin preservim/nerdtree plugins
install_plugin preservim/nerdcommenter plugins
# install_plugin rakr/vim-one.git colors
# install_plugin sainnhe/edge colors
install_plugin catppuccin/vim colors
install_plugin vim-airline/vim-airline colors
install_plugin luochen1990/rainbow syntax
install_plugin Yggdroot/indentLine syntax
