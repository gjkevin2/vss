...
grep "upstream singbox-hysteria2" /etc/nginx/nginx.conf || {
  sed -i "/\$ssl_preread_server_name/a\\\t\thysteria2.$servername singbox-hysteria2;" /etc/nginx/nginx.conf
  sed -i "/upstream set/a\\\tupstream singbox-hysteria2 {\n\t\tserver 127.0.0.1:52004;\n\t}" /etc/nginx/nginx.conf
}
grep "upstream singbox-tuic" /etc/nginx/nginx.conf || {
  sed -i "/\$ssl_preread_server_name/a\\\t\ttuic.$servername singbox-tuic;" /etc/nginx/nginx.conf
  sed -i "/upstream set/a\\\tupstream singbox-tuic {\n\t\tserver 127.0.0.1:52005;\n\t}" /etc/nginx/nginx.conf
}
